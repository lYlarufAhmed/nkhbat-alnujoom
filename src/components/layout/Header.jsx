...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 h-16 lg:h-20 glass z-50 px-4 lg:px-8 xl:px-12 flex items-center justify-between">
      {/* Left Side: Back button (sub/detail pages) or Settings icon (home screen) */}
      {!isMainTab ? (
        <button
          onClick={goBack}
          className="w-9 h-9 lg:w-10 lg:h-10 flex items-center justify-center rounded-xl bg-bg-surface border border-border text-text-primary hover:bg-accent/10 hover:text-accent transition-colors active:scale-90"
        ...
        <button
          ...
          className="w-9 h-9 lg:w-10 lg:h-10 flex items-center justify-center rounded-xl bg-bg-surface border border-border text-text-secondary hover:bg-accent/10 hover:text-accent hover:border-accent/30 transition-all duration-200"
      ...
      <h1 className="text-base lg:text-lg xl:text-xl font-bold text-text-primary tracking-wide">
